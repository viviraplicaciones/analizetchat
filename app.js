...
function shareAppAction() {
    if(navigator.share) {
        navigator.share({
            ...
            text: 'Analiza tus chats de forma segura y local.',
...
function sendBugEmail() {
    ...
    const subject = encodeURIComponent("Reporte de Fallo - Analizador WA");
    const body = encodeURIComponent(`Hola, encontré el siguiente fallo en la aplicación:\n\n${description}\n\nDispositivo: ${navigator.userAgent}`);
...
async function getGeneratedDoc() {
    ...
    doc.text("Reporte de WhatsApp", 105, y, { align: "center" });
    y += 15;

    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    doc.text(`Conversación: ${participants.join(' & ')}`, 14, y);
    y += 8;
    doc.text(`Total Mensajes: ${currentMessages.length}`, 14, y);
    y += 8;
    ...
    doc.text(`- ${participants[0]}: ${document.getElementById('stat-p1').innerText} mensajes`, 14, y);
    y += 6;
...
async function saveSlot(data) {
    ...
    await new Promise((resolve, reject) => {
        const req = tx.objectStore('slots').put(data);
...
async function updateSlotsUI() {
    ...
    if(slots.length === 0) {
        ui.slotsContainer.innerHTML = '<p style="text-align:center;color:var(--color-text-secondary);font-size:0.85rem;padding:20px;">No tienes conversaciones guardadas.</p>';
    }
    slots.forEach(slot => {
        const div = document.createElement('div');
        ...
        div.innerHTML = `
            <div class="slot-info">
                <h4>${slot.name}</h4>
                <p>${slot.date} • ${slot.msgs.length} msgs</p>
            </div>
...
function handleSplashScreen() {
    ...
    document.getElementById('no-splash').addEventListener('change', (e) => {
        localStorage.setItem('hideSplash', e.target.checked);
    });
}
